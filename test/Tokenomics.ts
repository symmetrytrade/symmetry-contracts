...
describe("tokenomics", () => {
    ...
    async function userVestBalanceAt(account: ethers.Signer, ts: number) {
        const n = (await votingEscrow_.userVestEpoch(await account.getAddress())).toNumber();
        let ans = new BigNumber(0);
        for (let i = 1; i <= n; ++i) {
            const vest = await votingEscrow_.userVestHistory(await account1.getAddress(), i);
            if (vest.ts > ts) {
                ans = ans.plus(
                    new BigNumber(vest.amount.toString()).dividedToIntegerBy(maxTime).multipliedBy(vest.ts - ts)
                );
            }
        }
        return ans;
    ...
    it("withdraw 100 lp", async () => {
        ...
        expect(await votingEscrow_.balanceOf(await account1.getAddress())).to.deep.eq(totalSupply.toString(10));
        expect(
            await votingEscrow_.balanceOfAt(await account1.getAddress(), (await helpers.time.latest()) - WEEK)
        ).to.deep.eq((await userVestBalanceAt(account1, (await helpers.time.latest()) - WEEK)).toString(10));
        expect(await votingEscrow_.totalSupplyAt((await helpers.time.latest()) - WEEK)).to.deep.eq(
            (await userVestBalanceAt(account1, (await helpers.time.latest()) - WEEK)).toString(10)
        );
    });
    ...
    it("lock SYM, trigger callback and vest", async () => {
        ...
        expect(veBalance.toString(10)).to.deep.eq(await votingEscrow_.totalSupply());
        expect(veBalance.toString(10)).to.deep.eq(await votingEscrow_.balanceOf(await account1.getAddress()));
        expect(
            ...
            (await userVestBalanceAt(account1, (await helpers.time.latest()) + 100))
                .plus(
                    new BigNumber(normalized(100))
                        .dividedToIntegerBy(maxTime)
                        .multipliedBy(lockEnd - (await helpers.time.latest()) - 100)
                )
                .toString(10)
        );
    });
    it("stake SYM, trigger callback and vest", async () => {
        ...
        const balance2 = (await userVestBalanceAt(account1, evmTime - 2 * WEEK)).toString(10);
        ...
        expect(stakePoint.slope).to.deep.eq(new BigNumber(normalized(100)).dividedToIntegerBy(-maxTime));
        ...
        expect(await votingEscrow_.totalSupply()).to.deep.eq(
            (await userVestBalanceAt(account1, await helpers.time.latest()))
                .plus(new BigNumber(normalized(100)).dividedToIntegerBy(maxTime).multipliedBy(WEEK * 3))
                .toString(10)
        );
    ...
    it("12 weeks later, stake more", async () => {
        ...
        expect(stakePoint.slope).to.deep.eq(new BigNumber(normalized(200)).dividedToIntegerBy(-maxTime));
        expect(stakePoint.ts).to.deep.eq(evmTime);
        ...
        // check balance
        expect(await votingEscrow_.totalSupply()).to.deep.eq(
            (await userVestBalanceAt(account1, await helpers.time.latest())).plus(veGot).toString(10)
        );
        expect(await votingEscrow_.balanceOf(await account1.getAddress())).to.deep.eq(
            (await userVestBalanceAt(account1, await helpers.time.latest())).plus(veGot).toString(10)
        );
        ...
        veGot = new BigNumber(veGot)
            .plus(new BigNumber(normalized(200)).dividedToIntegerBy(maxTime).multipliedBy(WEEK * 3))
            .toString(10);
        expect(await votingEscrow_.totalSupply()).to.deep.eq(
            (await userVestBalanceAt(account1, await helpers.time.latest())).plus(veGot).toString(10)
        );
        ...
        const balance2 = (await userVestBalanceAt(account1, time2))
            .plus(
                new BigNumber(veGot).plus(new BigNumber(normalized(200)).dividedToIntegerBy(maxTime).multipliedBy(1000))
            )
            .toString(10);
        // till maximum
    ...
    it("unstake", async () => {
        ...
        await expect(votingEscrow_.connect(account1).unstake(normalized(100)))
            ...
            .withArgs(await account1.getAddress(), normalized(100), evmTime);
        expect(await votingEscrow_.totalSupply()).to.deep.eq(
            (await userVestBalanceAt(account1, await helpers.time.latest())).toString(10)
        );
        ...
        await increaseNextBlockTimestamp(WEEK); // 1 week
        await (await votingEscrow_.connect(account1).claimVested(await account1.getAddress())).wait();
        const veGot = new BigNumber(normalized(100)).dividedToIntegerBy(maxTime).multipliedBy(WEEK);
        expect(await votingEscrow_.totalSupply()).to.deep.eq(
            (await userVestBalanceAt(account1, await helpers.time.latest())).plus(veGot).toString(10)
        );
        expect(await votingEscrow_.balanceOf(await account1.getAddress())).to.deep.eq(
    ...
    it("stake, increase, withdraw", async () => {
        ...
        let veLock = new BigNumber(normalized(100)).dividedToIntegerBy(maxTime).multipliedBy(1000);

        expect(await votingEscrow_.totalSupply()).to.deep.eq(veLock.plus(veStake).toString(10));
        expect(await votingEscrow_.balanceOf(await account1.getAddress())).to.deep.eq(veStake.toString(10));
        expect(await votingEscrow_.balanceOf(await account2.getAddress())).to.deep.eq(veLock.toString(10));

        expect(await votingEscrow_.totalSupplyAt((await helpers.time.latest()) + 10000)).to.deep.eq(
        ...
        veLock = new BigNumber(normalized(400)).dividedToIntegerBy(maxTime).multipliedBy(lockEnd - evmTime);
        expect(await votingEscrow_.totalSupply()).to.deep.eq(veLock.plus(veStake).toString(10));
        expect(await votingEscrow_.balanceOf(await account1.getAddress())).to.deep.eq(veStake.toString(10));
        expect(await votingEscrow_.balanceOf(await account2.getAddress())).to.deep.eq(veLock.toString(10));
        ...
        evmTime += 1000;
        veLock = new BigNumber(normalized(400)).dividedToIntegerBy(maxTime).multipliedBy(lockEnd - evmTime);
        expect(await votingEscrow_.totalSupplyAt(evmTime)).to.deep.eq(veLock.plus(veStake).toString(10));
        expect(await votingEscrow_.balanceOfAt(await account1.getAddress(), evmTime)).to.deep.eq(veStake.toString(10));
        expect(await votingEscrow_.balanceOfAt(await account2.getAddress(), evmTime)).to.deep.eq(veLock.toString(10));
